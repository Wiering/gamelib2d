...
class ModPlayer
{
    ...
    public function parseData(data:ByteArray)
    {
		...
		if (xm) {
			...
			var uiBlockLen = data.readUnsignedInt();  // length of pattern block
			xtrace ("block length: " + uiBlockLen);
			nextpos = data.position + Std.int (uiBlockLen) - 4;
        ...
        for (i in 0...patcount) 
		{
			...
			if (xm) 
			{
				...
				xtrace ("blocklen=" + blocklen);
				var packType = data.readUnsignedByte();  // pack type
				xtrace ("packtype=" + packType);
            ...
            for (r in 0...rowcount) 
			{
                for (ch in 0...chancount) 
				{
					...
					{  //*
						...
						//if (traceAll) xtrace ("ins=" + ins + " vol=" + vol + " effect=" + effect + " effectarg=" + effectarg);
    ...
    public function calcSegment():Bool
    {
        ...
        while (true) {
			...
			if (stereo) {
				...
				if (dyn)
				{
					sde.data.writeFloat (mixedLeft / 32768.0);
					sde.data.writeFloat (mixedRight / 32768.0);
				}
				else
				{
				...
				if (dyn)
				{
					sde.data.writeFloat (mixed / 32768.0);
					sde.data.writeFloat (mixed / 32768.0);
				}
				else
				{
...
*/
	...
	public function playAsset (asset: String)
	{
		var ba: ByteArray = Assets.getBytes (asset);
		parseData (ba);
		var self = this;
		genWaveform (function() { self.ready = false; } );
		ready = true;
    ...
    public function stop()
    {
        stopnow = true;
	#if flash
        openfl.media.SoundMixer.stopAll ();
	#end
    }
