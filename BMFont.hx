...
class BMFont
{
	...
	public function drawText (bd: BitmapData, s: String, xpos: Int, ypos: Int)
	{
		...
		var lastAsc: Int = -1;
		var line: Int = 0;
		var maxw: Int = textWidth (s);
		var w: Int = textWidth (s, line);
		var h: Int = textHeight (s);
		
		var bdTmp: BitmapData = new BitmapData (maxw, h, true, 0);
		var fdx: Int = 0;
		//if (align == ALIGN_RIGHT) fdx -= w;
		//if (align == ALIGN_CENTER) fdx -= (w >> 1);
		if (align == ALIGN_RIGHT) fdx = maxw - w;
		if (align == ALIGN_CENTER) fdx = (maxw - w) >> 1;
		var fdy: Int = 0;
		...
		fdx = 0;
		if (align == ALIGN_RIGHT) fdx = Std.int (maxw * scaleX);
		if (align == ALIGN_CENTER) fdx = Std.int ((maxw / 2) * scaleX);
		
	#if cpp
		bd.draw (bdTmp, new Matrix (scaleX, 0, 0, scaleY, (xpos - fdx), ypos), colorTransform);
	#else
		bd.draw (bdTmp, new Matrix (scaleX, 0, 0, scaleY, (xpos - fdx), ypos), colorTransform, blendMode, null, smooth);
	#end
	}
	...
	public static function stringToIntArray (s: String): Array<Int>
	{
		var ai: Array<Int> = new Array ();
		for (i in 0...s.length)
			ai.push (s.charCodeAt (i));
		return ai;
	}
	
	public function new (fnt: String, abd: Array<BitmapData>, ?ia: Array<Int> = null)
	{
		if (ia == null)
		{
			if (fnt.substr (0, 7) == "assets/")
			{
				baFnt = Assets.getBytes (fnt);				
			}
			else
			{
#if (!(cpp || neko))
...
*/
			...
			var len: UInt = baFnt.length;
			if (baFnt.position < len)
		...
		{
			var bd = abdImages[i];
			abdImages[i] = new BitmapData (bd.width, bd.height, true, 0);
			
			for (y in 0 ... abdImages[i].height)
				for (x in 0 ... abdImages[i].width)
				{
					var rgb: Int = bd.getPixel32 (x, y);
