...
class Obj
{
	...
	public var ignoreCollisionsTime: Int;
...
#if USE_TILESHEET
	private var lastTileSheetTile: Int;
	public var displayOrder: Int;
	private static var resortNeeded: Bool = false;
#end
							...
							anim: Int, animflags: Int, ?lyr: Layer, ?objflags: ObjectFlags)
	{
		...
		if (x == null) x = 0;
		if (y == null) y = 0;
		if (mapPosX == null) mapPosX = 0;
		if (mapPosY == null) mapPosY = 0;
#end
		...
		surface = new BitmapData (ts.scaledTileW, ts.scaledTileH, true, 0x0);
	...
	public static function sort ()
	{
		...
		if (resortNeeded)
		{
			var a: Array<Obj> = new Array ();
			for (o in objlist)
				a.push (o);
		
			for (j in 0...a.length)
				//if (a[j].displayOrder != 0)
					for (i in j + 1...objlist.length)
						//if (a[i].displayOrder != 0)
							if (a[i].displayOrder < a[j].displayOrder)
							{
								var o: Obj = a[i];
								a[i] = a[j];
								a[j] = o;
								//trace (i + " " + j + "  " + a[i].displayOrder + " " + a[j].displayOrder);
							}
	...
	public function clearOwnMessages (?msg: String = "")
	{
		for (m in msglist)
		{
			if (m.source == this)
	...
	public static function checkObjectFocus (xview: Float, yview: Float): Point
	{
		...
		return Def.makePoint (xview, yview);
	}


	public function changeDepth (newParent)
	{
		screen.removeChild (bitmap);
		screen = newParent;
		screen.addChild (bitmap);
	#if USE_TILESHEET
		resortNeeded = true;
	...
	public function checkLevelBounds ()
	{
		if ((layer != null) && (! simple))
		{
			...
			if (YBlock)
			{
				var vertSupport: Int = 0;
				var vsCnt: Int = 0;
				for (i in oldMapX1...oldMapX2 + 1)
				{
					var b = (layer.readBoundMap (i, NewY, EdgeBnd, bounceGroup, 0x0F) & YBnd);
					Bound |= b;
					touchTile (i, NewY, YBnd);
					var x1: Int = Std.int (Math.max (XPos1, i * W));
					var x2: Int = Std.int (Math.min (XPos2, i * W + W - 1));
					if (b & Def.UPPER_BOUND != 0)
						vertSupport += x2 - x1;
					vsCnt += x2 - x1;
				}
				if (Bound & YBnd != 0) YBump = true;
				if (YBump)
					if (vertSupport / vsCnt <= 0.5)
						onAtEdge ();
			}
...
*/
	...
	public function onScreen (): Bool
	{
		...
		{
			xp = (x + shiftX + sizeX / 2) * layer.scaleX;
			yp = (y + shiftY + sizeY / 2) * layer.scaleY;
			
			if (! objFlags.staticObject)
			{
				xp -= (layer.scrollX () + layer.shiftX - layer.timeCounter * layer.timeX) / ts.upScaleX;
				yp -= (layer.scrollY () + layer.shiftY - layer.timeCounter * layer.timeY) / ts.upScaleY;
			}
			result = (xp > -layer.ts.tileW) && (xp < screenw + layer.ts.tileW) &&
		...
		if ((active) && (waitTime == 0) && (moveDelay == 0))
		{
			...
			if (animation - 1 >= 0)
			{
				...
				if (animationTimeCounter >= ts.getAnimationFrameCount (animation - 1))
				{
					...
					{
						onAnimation ();
						var fc = ts.getAnimationFrameCount (animation - 1);
						if (animation - 1 >= 0 && fc != 0)
							animationTimeCounter = animationTimeCounter % fc;
					}
				}
			}
			...
			if (blinkTime > 0)
				blinkTime--;
			if (ignoreCollisionsTime > 0)
				ignoreCollisionsTime--;
	...
	public function draw ()
	{
		if ((active) && 
			(waitTime == 0) && 
			((animation > 0) || (objFlags.tileInsteadOfAnimation && (animation >= 0))) && 
			(logicFrame > 0))
		{
			if (! ((blinkTime > 0) && (Def.globalFrameCounter % 2 == 0)))
			{
				...
				var tile = animation;
				if (!objFlags.tileInsteadOfAnimation)
					tile = ts.getAnimationFrame (animation - 1, animationTimeCounter);
				...
				if (ts.useDrawTiles)
				{
					var tileSheetTile = lastTileSheetTile;
			
					if ((tile != lastTile) || (flags != lastFlags) || redrawBitmap)
					{
						if (ts.megaTileNumber != null)
						{
							tileSheetTile = (tile == 0)? -1 : ts.megaTileNumber[m][tile - 1];
							lastTile = tile;
							lastFlags = flags;
							redrawBitmap = false;
						}
					...
					if (tileSheetTile > -1)
						if (layer != null)
							TileSet.addTileToDraw (layer, screen, Std.int (ts.upScaleX * (xp - layer.scaleX * ts.tileW / 2 + 0.95)), 
																  Std.int (ts.upScaleY * (yp - layer.scaleY * ts.tileH / 2 + 0.95)), 
																	tileSheetTile, alpha, (layer.scaleX * bitmap.scaleX + layer.scaleY * bitmap.scaleY) / 2.0);
						
					lastTileSheetTile = tileSheetTile;
				}
				else
		#end
				{
					...
					{
						if (tile == 0)
							surface.fillRect (Def.makeRect (0, 0, ts.scaledTileW, ts.scaledTileH), 0);
						else
							if (ct == null)
							{
				#if js
								surface.fillRect (Def.makeRect (0, 0, ts.scaledTileW, ts.scaledTileH), 0x00000000);
				#end
								ts.drawTile (surface, 0, 0, tile - 1, m);
								//surface.fillRect (Def.makeRect (0, 0, ts.tileW, ts.tileH), 0x80FFFFFF);
							}
							else
							{
								surface.fillRect (Def.makeRect (0, 0, ts.scaledTileW, ts.scaledTileH), 0);
								var bdTmp: BitmapData = new BitmapData (ts.scaledTileW, ts.scaledTileH, true, 0x0);
								ts.drawTile (bdTmp, 0, 0, tile - 1, m);
								//surface.fillRect (Def.makeRect (0, 0, ts.scaledTileW, ts.scaledTileH), 0x80FF00FF);
						...
						//   surface.fillRect (new Rectangle (0, 0, ts.scaledTileW, ts.scaledTileH), 0x80FFFFFF);
						lastTile = tile;
						lastFlags = flags;
						redrawBitmap = false;
					}
					...
					bitmap.alpha = alpha;
					bitmap.x = Std.int (ts.upScaleX * (xp - sx * ts.tileW / 2) + 0.95);  // FRACTION ???
					//bitmap.x = -0.374999;
					bitmap.y = Std.int (ts.upScaleY * (yp - sy * ts.tileH / 2) + 0.95);
					
					if (animation == Def.BIGINT)  // show bounding box
					{
						bitmap.bitmapData = new BitmapData (sizeX, sizeY, true, 0xFFFFFCC0);
						bitmap.x = x - ((layer.scrollX() + layer.shiftX - layer.timeCounter * layer.timeX) / ts.upScaleX);
						bitmap.y = y - ((layer.scrollY() + layer.shiftY - layer.timeCounter * layer.timeY) / ts.upScaleY);
						bitmap.scaleX = 1.0;
						bitmap.scaleY = 1.0;
					}
				}
...
*/
	...
	public function checkCollisions (Collisions: Array<Collision>)
	{
		   ...
		   )
		{
			...
			for (obj in ol)
				if ((obj != this) && (obj.active) && (! obj.ignore) && (obj.waitTime == 0) && (obj.ignoreCollisionsTime == 0))
				{
					if ((objFlags.staticObject == obj.objFlags.staticObject) &&
						 (
						//	((bounceGroup & obj.group) != 0) 
	...
	public function kill ()
	{
		...
		if ((tileToPutBack >= 0) && (layer != null))
		{
			...
			layer.tilesToRedraw.add (Def.makePoint (mapX, mapY));
		}
		deleteObj ();
	}
