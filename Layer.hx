...
// TODO - boundmap bug: cannot stand on top tile
...
class Layer
{
	...
	public var redrawAll: Bool;
	
#if USE_TILESHEET
	public var tileSheetData: Array<Array<Float>>;
	public var spriteOrder: Array<Sprite>;
#end
	
	
	public function activate (b: Bool)
	{
		if (surface != null) surface.fillRect (Def.makeRect (0, 0, bufW * ts.scaledTileW, bufH * ts.scaledTileH), 0x0);
		
		if (lyr != null) lyr.visible = b;
		if (mc00 != null) mc00.visible = b;
		if (mc01 != null) mc01.visible = b;
		if (mc10 != null) mc10.visible = b;
		if (mc11 != null) mc11.visible = b;
							...
							?hscroll: Bool, ?vscroll: Bool)
	{
		...
		if (xscale != null)  scaleX = xscale / ts.upScaleX;  else  scaleX = 1.0;
		if (yscale != null)  scaleY = yscale / ts.upScaleY;  else  scaleY = 1.0;

		timeX = 0;
		timeY = 0;

		tileW = ts.scaledTileW * scaleX;
		tileH = ts.scaledTileH * scaleY;
		...
		surface = new BitmapData (bufW * ts.scaledTileW, bufH * ts.scaledTileH, transparent, 0x0);
	...
	public function writeMap (x: Int, y: Int, tile: Int)
	{
		...
		mapData[y][x] = tile;
		tilesToRedraw.add (new Point (x, y)); // (Def.makePoint (x, y));
	}
	
	
	public function setMapCache (x: Int, y: Int, tile: Int)
	{
		if (mapScreenTiles != null)
			if (mapScreenTiles.length > y)
				if (mapScreenTiles[y].length > x)
					mapScreenTiles[y][x] = tile;
	}


	public function readMap (x: Int, y: Int): Int
	{
		return mapData[y][x];
	}
	...
	{
		if (boundMapData != null && x >= 0 && y >= 0)
		{
			//trace("" + (boundMapData[y][x]>>8) + " " + (boundMapData[y][x] & 0xFF));
			if (code != -1)
			...
			{
				if (mapCodeList != null)
					mapCodeList.add (new Point (x, y)); // (Def.makePoint (x, y));
			}
		}
	}
	...
	public function draw (?force: Bool = false)
	{
						...
						if (alwx || alwy || (oldtile & Def.TF_SEQUENCE != 0))
						{
							...
							if (count < limit || force)
							{
								if ((curTile | curFlags) != oldtile || force)
								{
									...
									{
							#if js
										surface.fillRect (Def.makeRect (bufx * ts.scaledTileW, bufy * ts.scaledTileH, ts.scaledTileW, ts.scaledTileH), 0x00000000);
							#end
										ts.drawTile (surface, bufx * ts.scaledTileW, bufy * ts.scaledTileH, curTile - 1, m);
									}
									else
										if (oldtile != 0)
											surface.fillRect (Def.makeRect (bufx * ts.scaledTileW, bufy * ts.scaledTileH, ts.scaledTileW, ts.scaledTileH), 0x00000000);
		...
		//for (i in 0...250)
		//{
		//	surface.setPixel32 (i, i, 0xFF000000 + i);
		//}
	...
	public function getLevelBitmap ()
	{
		return mc01;
	}
...
class SimpleTiledMapInfo extends MapData
{
